...
const COURSES={
  ...
  ru:{id:'ru',name:'Русский',short:'RU',flag:'🇷🇺',file:'ru_words.json',voice:'ru-RU',voiceAlt:'ru-RU',displayCount:137,countLabel:'137 A1 kelime ve ifade',targetLabel:'RUSÇA',starter:true},
...
function nextStudy(){
  ...
  if(session.autoSpeak&&(session.questionType==='dictation'||(!['tr-en','cloze','sentence'].includes(session.questionType)&&!session.hiddenMode))){
...
function renderStudyQuestion(){
  ...
  $('#studySpeak').hidden=['tr-en','cloze','sentence'].includes(mode);
...
async function initV40(){
  ...
  setupEvents();setupV40Events();updateCourseUI();renderAll();renderWords(true);registerServiceWorker();initFirebase();$('#installBtn').hidden=isStandalone();updateSelectedControls();
