...
const CONTENT_FILES={
  ...
  'stories:all':'stories.json','ru:alphabet':'ru_alphabet.json','ru:grammar':'ru_grammar.json','ru:dialogues':'ru_dialogues.json'
};
...
exports.getContentPack=onCall({region:REGION,enforceAppCheck:true,cors:true,maxInstances:20,timeoutSeconds:30},async request=>{
  ...
  const key=kind==='course'?`course:${course}`:kind==='stories'?'stories:all':kind==='alphabet'?'ru:alphabet':kind==='grammar'?'ru:grammar':kind==='dialogues'?'ru:dialogues':'';
  ...
  return {version:'5.0.0',kind,course,offset,limit,total:filtered.length,items:filtered.slice(offset,offset+limit)};
});
