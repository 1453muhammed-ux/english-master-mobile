...
async function syncCloudNow(){
  ...
  try{
    ...
    const publicName=accountDisplayName(profile?.name,authUser),userPayload={profile:{name:publicName,email:authUser.email||'',goal:Number(profile?.goal||20),voiceAccent:profile?.voiceAccent||'en-US',v5Voices:profile?.v5Voices||{},v5Rates:profile?.v5Rates||{},photoURL:authUser.photoURL||'',activeCourse,joinedAt:states.en?.stats?.joinedAt||state.stats.joinedAt||''},coursesJson:JSON.stringify(packed),courseSchema:1,clientUpdatedAt:String(state.lastActive||new Date().toISOString()),updatedAt:window.firebase.firestore.FieldValue.serverTimestamp()};
