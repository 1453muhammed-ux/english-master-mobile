...
function renderV431LanguageXp(){
  ...
  grid.innerHTML=COURSE_IDS.map(id=>{const m=v431CourseMetrics(id),active=id===activeCourse;
    return `<button type="button" class="language-xp-card course-${id} ${active?'active':''}" data-language-xp-course="${id}" aria-pressed="${active}"><span class="language-xp-flag">${m.meta.flag}</span><div class="language-xp-copy"><small>${esc(m.meta.name)}</small><strong>${Math.round(m.all.points)} XP</strong><p>Bugün ${Math.round(m.today.points)} XP · ${m.all.answers?`%${m.accuracy} doğruluk`:'ilk çalışma bekleniyor'}</p><i><em style="width:${m.coverage}%"></em></i></div><b>${m.meta.short}</b></button>`;
  }).join('');
}
function renderV431CourseLabels(){
  ...
  if($('#activeCourseXpLabel'))$('#activeCourseXpLabel').textContent=`${meta.name.toLocaleUpperCase('tr-TR')} KURS XP`;
  ...
  if($('#activeCourseXpText'))$('#activeCourseXpText').textContent=`${Math.round(m.all.points)} XP`;
  if($('#globalXpText'))$('#globalXpText').textContent=`${global} XP`;
  if($('#xpLevel'))$('#xpLevel').textContent=level;
  if($('#xpToNext'))$('#xpToNext').textContent=`${within===0&&m.all.points>0?500:500-within} XP`;
  if($('#xpLevelFill'))$('#xpLevelFill').style.width=`${within/5}%`;
  if($('#xpText'))$('#xpText').textContent=`${meta.short} ${Math.round(m.all.points)} XP`;
  if($('#progressCourseBadge'))$('#progressCourseBadge').textContent=`${meta.flag} ${meta.name} · ${Math.round(m.all.points)} XP`;
}
...
renderV43CoursePerformance=function(){
  ...
  grid.innerHTML=COURSE_IDS.map(id=>{const m=v431CourseMetrics(id),active=id===activeCourse;
    return `<article class="course-performance-card ${active?'active':''}"><span>${m.meta.flag}</span><div><b>${esc(m.meta.name)}</b><small><strong>${Math.round(m.all.points)} XP</strong> · ${m.marked} / ${m.total} işaretli · ${m.all.answers?`%${m.accuracy} doğruluk`:'henüz cevap yok'}</small><i><em style="width:${m.coverage}%"></em></i></div><strong>%${m.coverage}</strong></article>`;
  }).join('');
};
