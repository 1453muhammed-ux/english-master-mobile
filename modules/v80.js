/* WordPilot v8.0.0 Tester Beta
   Conversation Coach 3.0, instant language-pair switching and course-specific leagues.
...
function wp80BrandShell(){const version=$('.version');if(version)version.textContent='v8.0.0 · Tester Beta';const hero=$('#wp71ConversationSpotlight');if(hero){const h=hero.querySelector('h2'),d=hero.querySelector('[data-wp71="description"]');if(h)h.textContent=wp80T('coach');if(d)d.textContent=wp80T('coachDesc')}const eye=$('#view-ai .section-title .eyebrow');if(eye)eye.textContent='CONVERSATION COACH 3.0';const pairIcon=$('#wp71PairButton em');if(pairIcon)pairIcon.textContent='⇄'}
...
/* ---------- Conversation Coach 3.0 controls and patient microphone ---------- */
function wp80EnsureCoachControls(){
  ...
  const eye=$('#view-ai .section-title .eyebrow');if(eye)eye.textContent='CONVERSATION COACH 3.0';const pairIcon=$('#wp71PairButton em');if(pairIcon)pairIcon.textContent='⇄';const title=$('#view-ai .section-title h1');if(title)title.textContent=wp80T('coach');const desc=$('#view-ai .section-title .muted');if(desc)desc.textContent=wp80T('coachDesc');
  ...
  const status=$('#wp65AiMicStatus');if(status&&!wp80Listening)status.textContent=wp80T('listenReady');const version=$('.version');if(version)version.textContent='v8.0.0 · Tester Beta';
}
...
v5SendAiMessage=async function(message,options={}){
  ...
  let reply;try{if(V5_SECURITY.aiEnabled)reply=await v5CloudAi(history,message);else throw new Error('LOCAL')}catch(error){reply=v5LocalCoachReply(message,{source});if(error?.message==='AUTH_REQUIRED')reply.note=(wp71Support()==='tr'?'Gelişmiş bulut AI için Google hesabıyla giriş gerekir.':'Cloud AI requires Google sign-in.')}
...
leagueLabel=function(period=leaderboardPeriod){const leagueCourse=wp80LeagueCourse(),course=COURSES[leagueCourse]?.name||leagueCourse,periodText=period==='daily'?(wp71LanguageProfile.ui==='tr'?'bugünkü':'today’s'):period==='weekly'?(wp71LanguageProfile.ui==='tr'?'bu haftaki':'this week’s'):period==='monthly'?(wp71LanguageProfile.ui==='tr'?'bu ayki':'this month’s'):(wp71LanguageProfile.ui==='tr'?'toplam':'total');return `${course} · ${periodText} XP`};
...
function wp80AfterInit(){
  ...
  const version=$('.version');if(version)version.textContent='v8.0.0 · Tester Beta';
