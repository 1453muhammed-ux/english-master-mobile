...
async function v5SendAiMessage(message){
  ...
  let reply;try{if(V5_SECURITY.aiEnabled)reply=await v5CloudAi(history,message);else throw new Error('LOCAL')}catch(error){reply=v5LocalCoachReply(message);if(error?.message==='AUTH_REQUIRED')reply.correction='Gerçek AI için bir hesapla giriş yapmalısın.'}
...
leagueLabel=function(period=leaderboardPeriod){const label=period==='daily'?'Bugünkü':period==='weekly'?'Bu haftaki':period==='monthly'?'Bu ayki':'Toplam';return `${COURSES[activeCourse].name} · ${label} PP’ye göre`};
