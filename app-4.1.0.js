...
const COURSES={
  ...
  ru:{id:'ru',name:'Русский',short:'RU',flag:'🇷🇺',file:'ru_words.json',voice:'ru-RU',voiceAlt:'ru-RU',displayCount:500,countLabel:'500 kontrollü A1 kelime ve ifade',targetLabel:'RUSÇA',starter:true},
...
/* WordPilot v4.1 Russian course extension.
...
COURSES.ru.displayCount=500;
COURSES.ru.countLabel='500 kontrollü A1 kelime ve ifade';
...
function renderRussianTopics(){
  ...
  grid.innerHTML=RUSSIAN_TOPICS.map((topic,index)=>`<article class="russian-topic-card">
    <button type="button" class="topic-open" data-russian-topic="${esc(topic)}"><span>${String(index+1).padStart(2,'0')}</span><div><b>${esc(topic)}</b><small>${counts.get(topic)||0} kayıt · A1</small></div></button>
...
updateCourseUI=function(){
  ...
  const ruCard=document.querySelector('[data-course="ru"] small');if(ruCard)ruCard.textContent='Türkçe anlatımlı · 500 kontrollü A1 kayıt';
...
chooseComprehensiveType=function(w){
  ...
  if(activeCourse==='ru'&&n>=3&&n<=12&&Math.random()<.22)return'ordering';
  return base;
};
...
renderStudyQuestion=function(){
  ...
  $('#studyQuestion').textContent=firstLine(w.translation)||'Rusça cümleyi doğru sıraya koy';
  $('#studyPron').textContent='Kelimelere dokunarak Rusça cümleyi oluştur.';
...
async function initV40(){
  ...
  setupEvents();setupV40Events();setupV41Events();updateCourseUI();renderAll();renderWords(true);registerServiceWorker();initFirebase();$('#installBtn').hidden=isStandalone();updateSelectedControls();
